...
def actions(n):
    while n >= 0:
        ...
        write_report("""
                        <br/> <br/> 
                        <p> <strong>3.</strong> &nbsp; 
                        Verify WiFi status - ADB checks the status and then prints the result on Google Search Bar.</p> 
                        <br/>
                    """)
        wifi_status()
        ...
        write_report("""
                        <br/> <br/> 
                        <p> <strong>4.</strong> &nbsp; 
                        Verify Airplane Mode status -  ADB checks the status and then prints the result on Google Search Bar </p> <br/>
                    """)
        ...
        '''
        start_package(CONTACTS_PACKAGE) # opens the Contacts Application using the Package Name
        time.sleep(LONG_WAIT)
        ...
        write_report("""
                    ...
                    """)
        take_screenshot("ContactsApp-" + datetime.now().strftime('%M:%S.%f')[:-4])
        ...
        time.sleep(LONG_WAIT)
        take_screenshot("ContactsList-" + datetime.now().strftime('%M:%S.%f')[:-4])
        time.sleep(LONG_WAIT)
        home()
...
def wifi_status():
    cmd = f"{ADB_PATH} adb {DEVICE_ID} shell dumpsys wifi | grep 'Wi-Fi is'"
    ...
    cmd = f"{ADB_PATH} adb {DEVICE_ID} shell \"input keyboard text '{process}'\""
    execute_process(cmd, "Inserting Input with Wifi Status")
    time.sleep(LONG_WAIT)


def airplane_mode_status():
    cmd = f"{ADB_PATH} adb {DEVICE_ID} shell dumpsys wifi | grep 'mAirplaneModeOn' "
...
def add_contact(n):
    time.sleep(SHORT_WAIT)
    perfom_tap(CONTACTS_APP_ADD_BTN_CORD_X, CONTACTS_APP_ADD_BTN_CORD_Y)  # add button

    # perfom_tap(556, 1047) # add google account alert  -> cancel

    cmd = f"{ADB_PATH} adb {DEVICE_ID} shell \"input keyboard text 'Seeb-{n}'\""
    ...
    cmd = f"{ADB_PATH} adb {DEVICE_ID} shell \"input keyboard text 'Gar-{n}'\""
    ...
    perfom_tap(CONTACTS_APP_SAVE_BTN_CORD_X, CONTACTS_APP_SAVE_BTN_CORD_Y)  # save button
    time.sleep(LONG_WAIT)
