...
def start_process():
    ...
    '''
    battery_status()
    REPORT += f"<br/> <br/> <p> <strong>3.</strong> &nbsp; Verify the current battery percentage - ADB checks the status and then prints the result on Google Search Bar. </p> <br/>"
    ...
    '''
    turn_on_bluetooth()
    REPORT += f"<br/> <br/> <p> <strong>4.</strong> &nbsp; Turn on bluetooth - ADB checks the status and then prints the result on Google Search Bar. </p> <br/>"
...
def battery_status():
    ...
    time.sleep(1)
    cmd = f"adb {DEVICE_ID} shell \"input keyboard text 'Battery {process}'\""
...
def turn_on_bluetooth():
    ...
    perfom_tap(542, 1700)  # google search bar
    time.sleep(1)
    cmd = f"adb {DEVICE_ID} shell \"input keyboard text 'Bluetooth: {process}'\""
...
def write_on_file(with_name, a_string):
    ...
    with open(with_name, "rt") as website:
        data = website.read()
        data = data.replace("IMAGES_HERE", a_string)
